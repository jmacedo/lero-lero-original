const phrasePiece0 = ["Caros amigos, ",
    "Por outro lado, ",
    "Assim mesmo, ",
...
const phrasePiece1 = ["a execução dos pontos do programa ",
...
const phrasePiece2 = ["nos obriga à análise ",
...
const phrasePiece3 = ["das condições financeiras e administrativas exigidas.",
...
function generateLeroLero(lines) {
    let leroLero = [], i = 0;

    while (lines > 0) {
        phrasePiece0.shuffle();
        phrasePiece1.shuffle();
        phrasePiece2.shuffle();
        phrasePiece3.shuffle();

        i = Math.floor(Math.random() * phrasePiece0.length);
        leroLero.push(phrasePiece0[i] + phrasePiece1[i] + phrasePiece2[i] + phrasePiece3[i]);
